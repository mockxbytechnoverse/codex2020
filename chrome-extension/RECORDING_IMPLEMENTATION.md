...
- **popup.html/popup.js**: Extension popup UI and main control logic
...
### Tab Recording
1. User clicks "Record tab" in popup
2. Popup shows 3-second countdown, then closes
3. Background script uses `chrome.tabCapture.getMediaStreamId()` to get stream ID
4. Stream ID is passed to content-recording.js via `START_RECORDING_WITH_STREAM_ID`
5. Content script creates MediaRecorder with tab capture constraints
6. Recording bar is shown on the active tab
7. Video data is collected and saved to server on stop

### Desktop Recording  
1. User clicks "Record desktop" in popup (no countdown - starts immediately)
2. Background script calls `chrome.desktopCapture.chooseDesktopMedia()` 
3. User selects screen/window to record
4. Background script handles entire setup via `handleDesktopRecordingSetup()`
5. Recording bars are injected into ALL tabs
6. Content-recording.js is injected only into active tab
7. Desktop recording starts with `START_DESKTOP_RECORDING_WITH_STREAM_ID`
8. Recording bars on all tabs are functional and control the same recording
